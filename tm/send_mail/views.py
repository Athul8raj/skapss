...
def format_mail(email,html_template,**kwargs):
    html_msg = render_to_string(html_template, kwargs)
    plain_message = strip_tags(html_msg) 
    if '@gmail.com' in email:
        email_from = settings.EMAIL_HOST_USER
    elif '@neviton.com' or 'outlook.com' in email:
        email_from = settings.EMAIL_HOST_USER
    recipient_list = [email]
    return plain_message, email_from, recipient_list,html_msg

@csrf_exempt
def pwd_reset_email(request,email):
    subject = f'TM- Reset Password Request'
    ...
    if token and user:
        ...
        plain_message, email_from, recipient_list,html_msg = format_mail(email,'mail_template.html', reset_url= reset_url,name=user.username)
