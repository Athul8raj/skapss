...
def check_token_validity(token):
    try:
        token = TM_USER_TOKEN.objects.get(token_id=token)
        if token.timestamp > timezone.now() - timedelta(hours=EXPIRE_HOURS):
            token.update(timestamp=timezone.now())
            return True,None,token
        elif token.timestamp - timezone.now() > timedelta(hours=1):
            user = User.objects.get(email=token.email)
            new_token = user_token.make_token(user)
            new_user_token = TM_USER_TOKEN(email=token.email)
            new_user_token.update(token_id=new_token,timestamp=datetime.datetime.utcnow().replace(tzinfo=pytz.utc))
            return False,new_token,token
        else:
